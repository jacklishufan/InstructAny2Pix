...
def get_ckpt_path(name, root, check=False):
    assert name in URL_MAP_AUDIO
    path = os.path.join(root, CKPT_MAP_AUDIO[name])
    if not os.path.exists(path) or (check and not md5_hash(path) == MD5_MAP_AUDIO[name]):
        print("Downloading {} model from {} to {}".format(name, URL_MAP_AUDIO[name], path))
        download(URL_MAP_AUDIO[name], path)
        md5 = md5_hash(path)
        assert md5 == MD5_MAP_AUDIO[name], md5
    return path
